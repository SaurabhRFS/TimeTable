...
const Navbar = ({ activeTab, setActiveTab }) => {
  ...
  // Colors are now exclusively used for the icon boxes
  const menuItems = [
    ...
    <>
      ...
      <nav className="fixed top-6 left-1/2 -translate-x-1/2 w-[95%] max-w-7xl z-50 hidden md:block">
        <div className="relative rounded-full border border-white/40 bg-white/40 backdrop-blur-2xl shadow-[inset_0_1px_0_0_rgba(255,255,255,0.5),0_8px_20px_rgba(0,0,0,0.05)] px-8 py-3 flex items-center justify-between transition-all hover:bg-white/50">
          ...
          <div className="absolute left-1/2 -translate-x-1/2 flex items-center justify-center leading-none z-10">
            <div className="flex items-center gap-2">
              {greeting.icon}
              <span className={`text-[12px] font-extrabold tracking-widest uppercase ${getGreetingStyle()}`}>{greeting.text}</span>
            </div>
          </div>
      ...
      <nav className="md:hidden fixed top-4 left-1/2 -translate-x-1/2 w-[92%] z-50">
        <div className="rounded-2xl border border-white/40 bg-white/40 backdrop-blur-2xl shadow-lg px-5 py-3 flex justify-between items-center relative overflow-hidden">
          ...
          <div className="flex flex-col leading-none z-10 text-left">
      ...
      {/* SLIDE-IN DRAWER */}
      <div className={`fixed top-0 left-0 h-full z-[70] w-[85%] md:w-[400px] bg-white/40 backdrop-blur-3xl border-r border-white/50 shadow-2xl transition-transform duration-200 cubic-bezier(0.4, 0, 0.2, 1) ${isMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="relative z-10 h-full flex flex-col p-6 overflow-y-auto">
          ...
          <div className="flex justify-between items-center mb-8">
            <div>
              ...
              <span className="text-xs font-bold text-slate-500 tracking-wider uppercase mt-1 block">Timetable Navigation</span>
            </div>
              ...
              return (
                <button
                    ...
                    className={`w-full p-4 rounded-2xl border transition-all duration-300 flex items-center gap-4 group outline-none ${
                        isActive 
                        ? 'bg-white/80 border-slate-300 transform scale-[1.02] shadow-md ring-1 ring-slate-200' 
                        : 'bg-white/40 border-white/60 hover:bg-white/70 hover:scale-[1.01] hover:shadow-sm'
                    }`}
                >
                    {/* Colorful Icon Box */}
                    ...
                    <div className="flex flex-col items-start">
                        <span className="font-black text-lg tracking-tight text-slate-800 group-hover:text-slate-900">
                            {item.label}
                        </span>
                    </div>
                </button>
